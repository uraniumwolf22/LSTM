...
class RecurrentNeuralNetwork
    ...
    def backProp(self)
        ...
        for i in range(self.rl, -1, -1)
            ...
            tu += np.dot(np.atleast_2d(error
                          self.dsigmoid(self.oa[i])),
...
class LSTM
    ...
    def backProp(
        ...
        ou = np.dot(np.atleast_2d(do  self.dtangent(o)).T,
                    np.atleast_2d(self.x))
        dcs = np.clip(e  o  self.dtangent(self.cs) + dfcs, -6, 6)
        dc = dcs  i
        cu = np.dot(np.atleast_2d(dc  self.dtangent(c)).T,
                    np.atleast_2d(self.x))
        di = dcs  c
        iu = np.dot(np.atleast_2d(di  self.dsigmoid(i)).T,
                    np.atleast_2d(self.x))
        df = dcs  pcs
        fu = np.dot(np.atleast_2d(df  self.dsigmoid(f)).T,
                    np.atleast_2d(self.x))
        dpcs = dcs  f
        dphs = np.dot(dc, self.c)[self.ys] + np.dot(do,
                self.o)[self.ys] + np.dot(di, self.i)[self.ys] 
            + np.dot(df, self.f)[self.ys]
        return (
            fu,
            iu,
            cu,
...
def LoadText()
    print 'Loading text'
    with open('info.txt', 'r') as text_file
        data = text_file.read()
    text = list(data)
    outputSize = len(text)
    data = list(set(text))
...
def main()
    ...
    (returnData, numCategories, expectedOutput, outputSize, data) = 
        LoadText()
    print 'Done Reading'
    RNN = RecurrentNeuralNetwork(numCategories, numCategories,
                                 outputSize, expectedOutput,
                                 learningRate)
